...
class Cloud115(object):
    ...
    def do_login(self):
        ...
        data = self.urlopen('http://passport.115.com/?ct=login&ac=qrcode_token')
        ...
        qrurl = 'http://qrcode.115.com/api/qrcode.php?qrfrom=1&&uid='+uid+'&_'+str(time.time())+'&_t='+str(time.time())
        qrShower = QRShower()
        ...
        data = self.urlopen('http://passport.115.com/?ct=login&ac=qrcode&key=' + key + '&v=' + v)
        data = self.fetch(data)

        #data = self.urlopen('http://www.115.com/?ct=login&ac=is_login&_=' + str(time.time()))
        data = self.urlopen('http://passport.115.com/?ct=ajax&ac=islogin&_=' + str(time.time()))
        data = self.fetch(data)
        data = json.loads(data[data.index('{'):])
        qrShower.close()
        qthread.join()
